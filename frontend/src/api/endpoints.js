...
const endpoint = {
    ...
    GET_CUSTOMER_BY_EMAIL: (email) =>
        `/suiteql/customer/by-email?email=${email}`,
    ...
    // ========== Recurring Orders (SuiteQL list/get) ==========
    GET_RECURRING_ORDERS_BY_CUSTOMER: ({
        customerId,
        limit,
        offset,
        timestamp, // optional cache-buster
    } = {}) => {
        const params = new URLSearchParams();
        if (customerId) params.append("customerId", customerId);
        if (limit) params.append("limit", limit);
        if (offset) params.append("offset", offset);
        if (timestamp) params.append("t", String(timestamp));
    ...
    // 🔽 ADD THESE TWO (same URL, different body)
