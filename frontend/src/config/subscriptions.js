...
export function parseStatus(rec) {
  ...
  const text =
    raw?.text ?? raw?.label ??
    rec?.statusText ?? rec?.status_text ??
    rec?.status ?? null;
...
export function pickNextRunDate(rec) {
  const raw =
    ...
    rec?.next_run ?? rec?.nextRun ?? rec?.nextrun ??
    rec?.custrecord_next_run ?? rec?.custrecord_ro_nextrun ?? null;
...
export function buildRecurringOrderPatch({
  ...
  if (interval != null) Object.assign(payload, buildIntervalPatchPayload(interval));
  if (date) payload.custrecord_ro_next_run = date;
  if (status && STATUS_PAYLOADS[status]) Object.assign(payload, STATUS_PAYLOADS[status]);
  return payload;
}
...
export async function enrichSubscriptionsWithPdp(list) {
  ...
  const enriched = await Promise.all(
    list.map(async (s) => {
      try {
        const candidate = s.displayname || s.itemid || s.productId || `#${s.roId}`;
